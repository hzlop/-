...
(function () {
    ...
    function checkGrowthArray(arr) {
        ...
        if (sorted.length <= 2) return { isValid: false, sortedArr: sorted };
    ...
    function cleanData(rawDict) {
        ...
        for (const key of Object.keys(rawDict)) {
            ...
            if (!isValid) continue;
            sortedArr.forEach(item => { item[0] = timestampToMonthDay(item[0]); });
    ...
    function formatMarkdownTable(map) {
        ...
        const [d1, d2, d3] = map[firstKey];

        let table = `| 款式 | ${d1[0]} | ${d2[0]} | ${d3[0]} |\n| ------ | ---- | ---- | ---- |\n`;
    ...
    /** 生成多维表格行数组（用于飞书表格） */
    function formatXlsxRows(map, shopName) {
        const firstKey = Object.keys(map)[0];
        const [d1, d2, d3] = map[firstKey];
    ...
    function sendFeishuMessage(text) {
        GM_xmlhttpRequest({
            ...
            onload:  res => console.log('Message sent:', res.status, res.responseText),
        });
    }
                    ...
                    GM_xmlhttpRequest({
                        ...
                        onload: async res => {
                            if (res.status < 200 || res.status >= 300) {
                                console.error('Webhook发送失败 (GM_xmlhttpRequest)', rows[i]);
                            }
    ...
    async function fetchSearchData() {
        ...
        while (page <= pageCount) {
            ...
            try {
                ...
                try {
                    let parsed    = JSON.parse(JSON.parse(jsonStr).ReturnValue);
                    ...
                    if (!pageData || pageData.length === 0) {
                        wordsEl.innerHTML = `[${shopNameById(currentShopID)}] 聚水潭服务器数据源第${page}页没有返回数据`;
                        page++;
                        await randomDelay();
                        continue;
                    }
                    ...
                    wordsEl.innerHTML = `[${shopNameById(currentShopID)}] 解析 JSON 失败，如有疑问请查看控制台`;
        ...
        queryBtn.addEventListener('click', async () => {
            ...
            try {
                for (const shopKey of selectedShops) {
                    ...
                    const xlsxRows = formatXlsxRows(checkedData, SHOPS[shopKey].Name);
                    console.log(msgText);

                    try {
                        sendFeishuMessage(msgText);
                        wordsEl.innerHTML = `${SHOPS[shopKey].Name} 快揽推送成功`;
                        await sendXlsxToFeishu(xlsxRows);
                        wordsEl.innerHTML = `${SHOPS[shopKey].Name} 表格推送成功`;
                    } catch (pushErr) {
                        wordsEl.innerHTML = `${SHOPS[shopKey].Name} 推送失败:${pushErr}`;
                    }
